...
function get20s(arr, from, to){

 let twenties = [];

 for(let i = 0; i < arr.length; i++){
   if(arr[i].years <= to && arr[i].years >= from){
     twenties.push(arr[i]);
   }
 }
...
function addArtist(arr, id, name, years, genre, nationality, bio,){
  ...
  console.log(addArtist(artists, 20, "Courtland", "1995 - current day", "Web Design", "American", "One day he baked cookies, another day he watched the sunset at Point Dume in Malibu."));
...
function randomize(/* Code here */){
 ...
 function artist1900(arr){
  let art = [];
  return arr.years >= "1900" && arr.years < "2000";
}
artist1900(artists);
art = artists.filter(artist1900);
console.log(art);
